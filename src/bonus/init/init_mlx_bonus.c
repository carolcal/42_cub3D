...
void	init_textures(t_game *game)
{
	...
	while (i < 4)
	{
		...
		init_texture(
			game->enemy_texture[i],
			game->enemy_tex_path[i],
			game->mlx
			);
		init_texture(game->goal_texture[i], game->goal_tex_path[i], game->mlx);
		i++;
	}
	init_texture(game->texture[i], game->map->door_tex_path, game->mlx);
}
...
static void	free_textures(t_game *game)
{
	...
	while (i < 5)
	{
		...
		if (i < 4 && game->enemy_texture[i] && game->enemy_texture[i]->tex_ptr)
		{
			mlx_destroy_image(game->mlx->mlx_ptr,
				game->enemy_texture[i]->tex_ptr);
			game->enemy_texture[i]->tex_ptr = NULL;
		}
		if (i < 4 && game->goal_texture[i] && game->goal_texture[i]->tex_ptr)
		{
			mlx_destroy_image(game->mlx->mlx_ptr,
